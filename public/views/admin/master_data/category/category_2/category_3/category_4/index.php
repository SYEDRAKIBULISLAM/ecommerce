...
<div class="page-container">
    ...
    <div class="page-content">
        ...
        <div class="page-title">
            <h2><button onclick="goBack()"><span class="fa fa-arrow-circle-o-left"></span></button> Category 4 <a href="<?=root()?>/admin/master_data/category/category_2/category_3/category_4/create" class="btn btn-primary"><i class="fa fa-plus"></i></a></h2>
        </div>
        <!-- END PAGE TITLE -->
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <table id="customers2" class="table datatable">
                                ...
                                <tbody>
                                <?php $sl = 1?>
                                <?php foreach ($data['category4'] as $item): ?>
                                    <tr id="<?=$item->id?>">
                                        <td><?=$sl?></td><?php $sl++?>
                                        <td><?=$item->name?></td>
                                        <td><?=$item->category3->name?></td>
                                        <td><?=$item->updated_at->format('Y-m-d')?></td>
                                        <td>
                                            <a href="<?=root()?>/admin/master_data/category/category_2/category_3/category_4/edit/<?=$item->id?>" class="btn btn-default btn-rounded btn-sm"><span class="fa fa-pencil"></span></a>
                                            <button class="btn btn-danger btn-rounded btn-sm" onClick="delete_row(<?=$item->id?>, '<?=root().'/admin/master_data/category/category_2/category_3/category_4/destroy/'.$item->id?>')"><span class="fa fa-times"></span></button>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                                </tbody>
